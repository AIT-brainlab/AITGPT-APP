...
# Inject Policy API URL into HTML for runtime configuration
if [ -n "$VITE_POLICY_API_URL" ]; then
    echo "Injecting Policy API URL: $VITE_POLICY_API_URL"
    if [ -f "/app/build/index.html" ]; then
        # Escape forward slashes and other special characters for sed
        ESCAPED_POLICY_URL=$(echo "$VITE_POLICY_API_URL" | sed 's|/|\\/|g' | sed 's|&|\\&|g')
        # Replace the default value in the script tag
        sed -i "s|window.__POLICY_API_BASE_URL__ = window.__POLICY_API_BASE_URL__ || 'http://localhost:8005'|window.__POLICY_API_BASE_URL__ = '$VITE_POLICY_API_URL'|g" /app/build/index.html 2>/dev/null || \
        sed -i "s|'http://localhost:8005'|'$VITE_POLICY_API_URL'|g" /app/build/index.html 2>/dev/null || \
        sed -i "s|http://localhost:8005|$VITE_POLICY_API_URL|g" /app/build/index.html 2>/dev/null

        if [ $? -eq 0 ]; then
            echo "✓ Policy API URL injected successfully: $VITE_POLICY_API_URL"
        else
            echo "⚠ Warning: Could not inject Policy API URL, using default or build-time value"
        fi
    else
        echo "⚠ Warning: index.html not found in /app/build/"
    fi
else
    echo "VITE_POLICY_API_URL not set, using default: http://localhost:8005"
fi

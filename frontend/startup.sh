...
if [ -n "$VITE_API_URL" ]; then
    ...
    if [ -f "${WORKDIR}/build/index.html" ]; then
        ...
        sed -i "s|window.__API_BASE_URL__ = window.__API_BASE_URL__ || 'http://localhost:8000'|window.__API_BASE_URL__ = '$VITE_API_URL'|g" ${WORKDIR}/build/index.html 2>/dev/null || \
        sed -i "s|'http://localhost:8000'|'$VITE_API_URL'|g" ${WORKDIR}/build/index.html 2>/dev/null || \
        sed -i "s|http://localhost:8000|$VITE_API_URL|g" ${WORKDIR}/build/index.html 2>/dev/null
